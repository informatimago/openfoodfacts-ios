#ifndef fr_sbde_protocol_error_h
#define fr_sbde_protocol_error_h
#include <stddef.h>
#include <errno.h>
...
extern out_of_memory_handler handle_out_of_memory;
...
extern error_handler handle_error;
...
extern warning_handler handle_warning;
#define WARN(status,format, ...) (handle_warning(__FILE__, __LINE__, __FUNCTION__, status, format, ## __VA_ARGS__))


typedef void (*verbose_handler)(const char* file, unsigned long line, const char* function, const char* format, ...);
extern verbose_handler handle_verbose;
...
/* checked_realloc
changes the buffer size, or if it can't, calls out_of_memory and return its results.
*/
